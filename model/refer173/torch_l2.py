...
test = os.listdir('../../dataset/test/test-01/')
...
for train_index, test_index in skf.split(train_feat, train_val['label']):
    ...
    train_loader = data_utils.DataLoader(train_set, batch_size=128, shuffle=True, num_workers=4)

    val_set = Arrayloader(X_test, y_test)
    val_loader = data_utils.DataLoader(val_set, batch_size=128, shuffle=True, num_workers=4)
    ...
    for epoch in range(epochsize):
        ...
        epoch_log = '[%d/%d] Loss %.6f/%.6f Acc %.6f/%.6f' % (
        epoch, epochsize, train_loss, val_loss, train_acc, val_acc)
        print(epoch_log)
...
print(feature_file)
print('+++ Loss %.6f/%.6f Acc %.6f/%.6f' % (np.mean(train_logs[0]), np.mean(train_logs[1]),
...
with codecs.open('ensemble_test4.txt', 'w') as f:
